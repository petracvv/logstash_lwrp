...
describe file('/opt/logstash_kitchen/config/logstash.yml') do
  ...
  its('content') { should match %r{path\.data: \/opt\/logstash_kitchen\/data} }
  its('content') { should match %r{path\.logs: \/opt\/logstash_kitchen\/logs} }
...
describe file('/opt/logstash_kitchen/config/pipelines.yml') do
  ...
  its('content') { should match %r{path.config: "\/opt\/logstash_kitchen\/pipelines\/main"} }
  its('content') { should match /pipeline.id: testing/ }
  its('content') { should match %r{path.config: "\/opt\/logstash_kitchen\/pipelines\/testing"} }
...
describe file('/opt/logstash_kitchen/config/startup.options') do
  ...
  its('group') { should eq 'logstash' }
  its('mode') { should cmp '0640' }
  its('content') { should match %r{JAVACMD=\/usr\/bin\/java} }
  its('content') { should match %r{LS_HOME=\/opt\/logstash_kitchen} }
  its('content') { should match %r{LS_SETTINGS_DIR=\/opt\/logstash_kitchen\/config} }
  its('content') { should match %r{LS_OPTS="--path.settings \/opt\/logstash_kitchen\/config"} }
  its('content') { should match %r{LS_PIDFILE=\/var\/run\/logstash_kitchen.pid} }
  its('content') { should match /LS_USER=logstash/ }
  its('content') { should match /LS_GROUP=logstash/ }
  its('content') { should match %r{LS_GC_LOG_FILE=\/opt\/logstash_kitchen\/logs\/logstash_kitchen_gc.log} }
